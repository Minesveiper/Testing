...
namespace Testing
{
    class Program
    {
        ...
        public static string AlfaStrip(string herodes)
        {
            ...
            //return new Regex(@"[aeiouyæøåAEIOUYÆØÅ]").Replace(alfabet, "");

            
            var nyAlfa = "";
            char[] vowel = new char[] { 'a', 'e', 'i', 'o', 'u', 'y', 'æ', 'ø', 'å' };

            for (int i = 0; i < herodes.Length; i++)
            {
                foreach (char vokal in vowel)
                {
                    if (herodes.Contains(vokal))
                    {
